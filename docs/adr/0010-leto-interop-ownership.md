...
- dense and one-dimensional Mnemosyne output constructors remain generic over
  element type and rank where the Leto operation permits it.
...
storage. This is a proof sketch over Leto's documented iterator contract, not
a machine-checked proof. Rank-one strided and rank-two transposed cases are
value-semantic regression evidence.
