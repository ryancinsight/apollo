...
1. `Shape1D`, `Shape2D` and `Shape3D` make their length fields private and
   ...
   `nx()`/`ny()`/`nz()`. The validating `new` constructors are the only
   construction path; the structs carry `#[non_exhaustive]`, and
   deserialization routes through the same check via a `try_from`
   representation, so a serialized zero never bypasses it either.
