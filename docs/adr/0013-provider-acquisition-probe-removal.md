...
Apollo transform crates expose `wgpu_available() -> bool`, and the FFT GPU
transport additionally exposed `gpu_fft_available() -> bool`. Each is an
unused public forwarding function that either evaluates its transform
backend's typed Hephaestus acquisition result with `is_ok()` or returns a
hardcoded `true`. The boolean erases the acquisition failure and duplicates a
provider concern across consumers.
...
Choose option 3. Delete every unused `wgpu_available` and
`gpu_fft_available` definition and root re-export in one pre-1.0 breaking
change, update every affected crate version, and keep no compatibility alias.
Consumers use the existing transform backend constructor and handle the
provider error. A shared Apollo helper would repeat the discarded
failure-erasure at a new location; generic device acquisition belongs to
Hephaestus.
