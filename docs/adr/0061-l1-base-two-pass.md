...
- **2026-09-15, the radix-8 column pass's spill traffic
  (`APOLLO-CHAIN-PASS-SPILLS`).** Two trials against the loop's eleven
  spill moves a chunk at eight lanes: the pass consuming its outputs
  pairwise (each final-stage butterfly's two outputs twiddled and stored
  as they form) and the eighths in the one-register half-root-two form
  instead of the four dup-split registers. Census
  (`scripts/codegen_attribution.py loops`): the baseline eight-lane loop 143 instructions a chunk, 114 vector, 27 loads and 9 stores of which 11 are spill moves, critical path 37 cycles against an issue bound of 29 to 38; trial one (pairwise consumption) 141 / 112 vector, 9 spill moves, critical path 44; trial two (pairwise and half-root-two eighths) 133 / 104 vector, 22 loads and 8 stores of which 6 are spill moves, 27 shuffles, critical path 45 against 26 to 35. Phases against the
