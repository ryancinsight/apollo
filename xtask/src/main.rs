...
const CANONICAL_SIZES: &[usize] = &[
    ...
    194, 195, 196, 197, 198, 199, 200, 10_007, 32_768,
];
...
fn bench_pair<P: PrecisionBenchmark>(len: usize, profile: BenchmarkProfile) -> (f64, f64) {
    ...
    let apollo = measure_operation(profile, || {
    ...
    let rust = measure_operation(profile, || {
        let mut data = rustfft_input.clone();
        rustfft.process_with_scratch(std::hint::black_box(&mut data), &mut rustfft_scratch);
        std::hint::black_box(data);
    });
