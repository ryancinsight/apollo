...
/// One size's four measurements, in picoseconds.
...
impl Row {
    ...
    fn cell(value: Option<u128>) -> String {
        value.map_or_else(
            || "-".to_owned(),
            |picoseconds| format!("{}.{:03}", picoseconds / 1_000, picoseconds % 1_000),
        )
    }
}
...
/// `case,min_ps,median_ps,...` where `case` is `group/operation/parameter`.
...
mod tests {
    ...
    const SAMPLE: &str = "case,min_ps,median_ps,median_lower_ps,median_upper_ps\n\
fft_forward_clone_inclusive/apollo_f64/8,10000,12000,12000,13000\n\
fft_forward_clone_inclusive/rustfft_f64/8,20000,24000,24000,25000\n\
fft_forward_clone_inclusive/apollo_f32/8,5000,6000,6000,7000\n\
fft_forward_clone_inclusive/rustfft_f32/8,10000,12000,12000,13000\n\
some_other_group/apollo_f64/8,99000,99000,99000,99000\n";
    ...
    fn parses_only_the_comparison_group() {
        ...
        let row = rows[&8];
        assert_eq!(row.apollo_f64, Some(12_000));
        assert_eq!(row.rustfft_f64, Some(24_000));
        assert_eq!(row.apollo_f32, Some(6_000));
        assert_eq!(row.rustfft_f32, Some(12_000));
    }
    ...
    fn rendered_table_has_one_row_per_size_in_ascending_order() {
        ...
        assert!(table.contains("| 8 | 12.000 | 24.000 | 0.500x | 6.000 | 12.000 | 0.500x |"));
        assert!(
