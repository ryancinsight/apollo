//! CZT accelerator error contracts.

use hephaestus_core::HephaestusError;
use thiserror::Error;

/// Result alias for CZT accelerator execution.
pub type WgpuResult<T> = Result<T, WgpuError>;

/// Failures produced by the concrete Hephaestus WGPU implementation of CZT.
#[derive(Debug, Error)]
pub enum WgpuError {
    ...
    /// Plan parameters are invalid for the direct CZT kernel.
