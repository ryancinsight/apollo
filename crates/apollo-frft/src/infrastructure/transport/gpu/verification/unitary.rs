...
use super::support::{
    assert_complex32_close, assert_cpu_differential, cpu_input, unitary_backend,
    UNITARY_VALUE_TOLERANCE,
};
use crate::infrastructure::transport::gpu::OrderPlan;

#[test]
fn forward_order_zero_is_identity() {
    let Some(backend) = unitary_backend() else {
        return;
    };
    let input = (0..8)
        .map(|index| {
        ...
        .collect::<Vec<_>>();
    let plan = UnitaryFrftWgpuPlan::new(OrderPlan::new(input.len(), 0.0_f32));
    let output = backend
...
fn leto_forward_and_inverse_match_slice_execution() {
    ...
    let input = (0..8)
        ...
        .collect::<Vec<_>>();
    let plan = UnitaryFrftWgpuPlan::new(OrderPlan::new(input.len(), 0.5_f32));
...
fn forward_order_two_is_reversal() {
    ...
    let input = (0..8)
        ...
        .collect::<Vec<_>>();
    let plan = UnitaryFrftWgpuPlan::new(OrderPlan::new(input.len(), 2.0_f32));
    let output = backend
        .execute_forward(&plan, &input)
...
fn forward_and_inverse_roundtrip_for_existing_orders() {
    ...
    for order in [0.3_f32, 0.5, 0.7, 1.3, 2.5, 3.1] {
        let plan = UnitaryFrftWgpuPlan::new(OrderPlan::new(input.len(), order));
...
fn gpu_matches_cpu_reference() {
    ...
    let input = (0..8)
        ...
        .collect::<Vec<_>>();
    let order = 0.5_f32;
    let gpu_plan = UnitaryFrftWgpuPlan::new(OrderPlan::new(input.len(), order));
    let actual = backend
