...
mod tests {
    ...
    #[test]
    fn dctdst_wgpu_execution_suite_when_device_exists() {
        let Ok(backend) = DctDstWgpuBackend::try_default() else {
            return;
        };

        // 1. backend_reports_dct_and_dst
        {
        ...
        // 2. dct2_forward_matches_cpu_reference
        {
            ...
            let cpu_plan =
                DctDstPlan::new(input.len(), RealTransformKind::DctII).expect("cpu plan");
            let cpu = cpu_plan
        ...
        // 3. dct3_forward_matches_cpu_reference
        {
            ...
            let cpu_plan =
                DctDstPlan::new(input.len(), RealTransformKind::DctIII).expect("cpu plan");
            let cpu = cpu_plan
                .forward(&input.iter().map(|&value| value as f64).collect::<Vec<_>>())
                .expect("cpu forward");
        ...
        // 5. dst2_forward_matches_cpu_reference
        {
            ...
            let cpu_plan =
                DctDstPlan::new(input.len(), RealTransformKind::DstII).expect("cpu plan");
            let cpu = cpu_plan
                .forward(&input.iter().map(|&value| value as f64).collect::<Vec<_>>())
                .expect("cpu forward");
        ...
        // 6. dst3_forward_matches_cpu_reference
        {
            ...
            let cpu_plan =
                DctDstPlan::new(input.len(), RealTransformKind::DstIII).expect("cpu plan");
            let cpu = cpu_plan
                .forward(&input.iter().map(|&value| value as f64).collect::<Vec<_>>())
                .expect("cpu forward");
        ...
        // 15. dct4_forward_matches_cpu_reference
        {
        ...
        // 16. dst1_forward_matches_cpu_reference
        {
        ...
        // 17. dst4_forward_matches_cpu_reference
        {
        ...
        // 18. dct1_inverse_recovers_input
        {
            let input = vec![1.0_f32, -0.5, 2.0, 0.75];
        ...
        // 19. dct4_inverse_recovers_input
        {
            let input = vec![0.5_f32, -1.0, 2.5, -0.25, 1.5, 0.75];
        ...
        // 20. dst1_inverse_recovers_input
        {
            let input = vec![1.0_f32, -2.0, 0.5, 3.0, -1.5];
        ...
        // 21. dst4_inverse_recovers_input
        {
            let input = vec![0.75_f32, -1.25, 2.0, -0.5, 1.0];
        ...
        // 22. dct1_rejects_length_less_than_two
        {
