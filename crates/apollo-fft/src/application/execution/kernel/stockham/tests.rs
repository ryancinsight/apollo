...
fn f32_avx_groups_eight_quad_stage_matches_scalar_reference() {
    ...
    let input: Vec<Complex32> = (0..n)
        ...
        .collect();
    let twiddles = crate::application::execution::kernel::radix2::build_forward_twiddle_table_32(n);
    let base = radix - 1;
...
fn f64_avx_groups_eight_triple_stage_matches_scalar_reference() {
    ...
    let input: Vec<Complex64> = (0..n)
        ...
        .collect();
    let twiddles = crate::application::execution::kernel::radix2::build_forward_twiddle_table_64(n);
    let base = radix - 1;
    let first = &twiddles[base..base + radix];
    let second = &twiddles[base + radix..base + 3 * radix];
    let third = &twiddles[base + 3 * radix..base + 7 * radix];
...
fn f64_hybrid_radix8x512_inverse_roundtrip_n4096() {
    ...
    let forward = crate::application::execution::kernel::radix2::build_forward_twiddle_table_64(n);
    let inverse = crate::application::execution::kernel::radix2::build_inverse_twiddle_table_64(n);
...
fn f32_hybrid_radix8x512_inverse_roundtrip_n4096() {
    ...
    let mut data: Vec<Complex32> = (0..n)
        ...
        .collect();
    let original = data.clone();
    let forward = crate::application::execution::kernel::radix2::build_forward_twiddle_table_32(n);
    let inverse = crate::application::execution::kernel::radix2::build_inverse_twiddle_table_32(n);
...
fn f64_avx_schedule_roundtrip_holds_for_n8192() {
    ...
    let forward =
        crate::application::execution::kernel::radix2::build_forward_twiddle_table_64(data.len());
    let inverse =
        crate::application::execution::kernel::radix2::build_inverse_twiddle_table_64(data.len());
