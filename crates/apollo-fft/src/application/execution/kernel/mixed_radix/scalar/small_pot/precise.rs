...
/// register form through [`small_pot_inplace_sized_precise_framed`]; every
...
>(
    ...
    #[cfg(target_arch = "x86_64")]
    {
        if matches!(N, 16 | 32 | 64) && super::super::simd::avx::vector_frame_available() {
            // SAFETY: the probe proved the frame on this host, and the caller's
            // length contract passes through unchanged.
            return small_pot_inplace_sized_precise_framed::<N, INVERSE, NORMALIZE>(data);
        }
    }
    small_pot_inplace_sized_precise_scalar::<N, INVERSE, NORMALIZE>(data);
}

/// Applies the codelet for `N` inside the caller's vector frame.
///
/// The frame is AVX2 and FMA, the same frame the reduced-density entry runs
/// in; the register arms here ([`super::n16`], [`super::n32`], and the 64-point
...
#[inline]
pub(in crate::application::execution::kernel::mixed_radix::scalar) unsafe fn small_pot_inplace_sized_precise_framed<
    const N: usize,
    const INVERSE: bool,
    const NORMALIZE: bool,
>(
    ...
    match N {
        ...
        16 => {
            ...
            super::n16::vector_arm::<INVERSE, NORMALIZE>(data);
        }
        #[cfg(target_arch = "x86_64")]
        32 => {
            // SAFETY: this frame establishes AVX and FMA, and the caller's
            // length contract fixes the span at thirty-two samples.
            let data_ref = &mut *data.as_mut_ptr().cast::<[Complex64; 32]>();
            super::n32::vector_arm::<INVERSE, NORMALIZE>(data_ref);
        }
        #[cfg(target_arch = "x86_64")]
        64 => {
            use std::arch::x86_64::{
        ...
        _ => small_pot_inplace_sized_precise_scalar::<N, INVERSE, NORMALIZE>(data),
    }
}
...
/// Carries the `MixedRadixScalar::small_pot_inplace_sized` contract unchanged.
#[inline]
unsafe fn small_pot_inplace_sized_precise_scalar<
    const N: usize,
    const INVERSE: bool,
    const NORMALIZE: bool,
>(
    ...
    match N {
        ...
        64 => {
            ...
            crate::application::execution::kernel::components::winograd::dft64_impl::<f64, INVERSE>(
                data_ref,
            );
            if INVERSE && NORMALIZE {
