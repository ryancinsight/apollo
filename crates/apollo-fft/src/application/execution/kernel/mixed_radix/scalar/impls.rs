        ...
        match n {
            2 => {
                #[cfg(all(target_arch = "x86_64", target_feature = "avx", target_feature = "fma"))]
                {
                    ...
                    let b_reg = _mm256_permute2f128_pd::<0x11>(reg, reg);
                    let sum = _mm256_add_pd(a_reg, b_reg);
                    let diff = _mm256_sub_pd(a_reg, b_reg);
                    let mut res = _mm256_permute2f128_pd::<0x20>(sum, diff);
                    if INVERSE && NORMALIZE {
                        let scale = _mm256_set1_pd(0.5);
                        res = _mm256_mul_pd(res, scale);
                    }
            ...
            8 => {
                #[cfg(all(target_arch = "x86_64", target_feature = "avx", target_feature = "fma"))]
                {
                    ...
                    let a1 = _mm_add_pd(x1, x5);
                    let mut a5 = _mm_sub_pd(x1, x5);
                    let a2 = _mm_add_pd(x2, x6);
                    ...
                    _mm_storeu_pd(ptr.add(6), x5);
                    _mm_storeu_pd(ptr.add(8), x2);
                    _mm_storeu_pd(ptr.add(10), x6);
                ...
                )))]
                {
                    ...
                    let diff15 = x1 - x5;
                    let a5 = if INVERSE {
                        Complex64::new((diff15.re - diff15.im) * c, (diff15.re + diff15.im) * c)
                    } else {
                        Complex64::new((diff15.re + diff15.im) * c, (diff15.im - diff15.re) * c)
                    };

                    let diff37 = x3 - x7;
                    let a7 = if INVERSE {
                        Complex64::new(-(diff37.re + diff37.im) * c, (diff37.re - diff37.im) * c)
                    } else {
                        Complex64::new((diff37.im - diff37.re) * c, -(diff37.re + diff37.im) * c)
                    };
                    ...
                    let diff26 = x2 - x6;
                    let a6 = if INVERSE {
                        Complex64::new(-diff26.im, diff26.re)
                    } else {
                        Complex64::new(diff26.im, -diff26.re)
                    };
