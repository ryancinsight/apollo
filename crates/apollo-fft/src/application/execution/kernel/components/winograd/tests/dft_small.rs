...
fn dft2_forward_matches_direct() {
    ...
    dft2_impl(&mut buf);
    assert!(max_err(&buf, &expected) < 1e-14, "DFT-2 forward mismatch");
}

#[test]
fn dft2_inverse_roundtrip() {
    let input = vec![Complex64::new(3.0, -1.0), Complex64::new(-2.0, 4.0)];
    let mut buf: [Complex64; 2] = input.as_slice().try_into().unwrap();
    dft2_impl(&mut buf);
    dft2_impl(&mut buf);
...
fn dft11_f32_forward_matches_direct() {
    ...
    let mut buf: [Complex32; 11] =
        core::array::from_fn(|i| Complex32::new(input[i].re as f32, input[i].im as f32));
    <f32 as ShortWinogradScalar>::dft11::<false>(&mut buf);
    let got: Vec<Complex64> = buf
        .iter()
        .map(|x| Complex64::new(x.re as f64, x.im as f64))
        .collect();
...
fn dft13_f32_forward_matches_direct() {
    ...
    let mut buf: [Complex32; 13] =
        core::array::from_fn(|i| Complex32::new(input[i].re as f32, input[i].im as f32));
    <f32 as ShortWinogradScalar>::dft13::<false>(&mut buf);
    let got: Vec<Complex64> = buf
        .iter()
        .map(|x| Complex64::new(x.re as f64, x.im as f64))
        .collect();
