...
pub(crate) unsafe fn stockham_quad_split_pair32(
    ...
    let product = cmul_pair_reduced(_mm_set1_ps(twiddle.re), _mm_set1_ps(twiddle.im), high);
...
pub(crate) unsafe fn stockham_quad_store_pair32(
    ...
    let product = cmul_pair_reduced(_mm_set1_ps(twiddle.re), _mm_set1_ps(twiddle.im), odd);
    store_reduced_low(dst_ptr.add(low_index), _mm_add_ps(pair, product));
...
pub(crate) unsafe fn stockham_quad_groups_eight32(
    ...
    for j in 0..radix {
        ...
        let x89ab = cmul_vec_reduced(
            w1r,
            w1i,
            _mm256_loadu_ps(src_ptr.add(src_base + 8).cast::<f32>()),
        );
        let xcdef = cmul_vec_reduced(
            w1r,
            w1i,
        ...
        let t4567 = cmul_vec_reduced(w20r, w20i, y4567);
        let tcdef = cmul_vec_reduced(w21r, w21i, ycdef);
