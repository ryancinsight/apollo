    ...
    C: Copy + std::ops::Add<Output = C> + std::ops::Sub<Output = C> + std::ops::Mul<Output = C>,
{
    ...
    let mut k_start = 0;
    while k_start < quarter_groups {
        let k_end = (k_start + TILE_SIZE).min(quarter_groups);
        for j in 0..radix {
...
#[inline]
pub(crate) fn stage_quad_impl<C, const TILE_SIZE: usize>(
    src: &[C],
    dst: &mut [C],
    radix: usize,
    first_twiddles: &[C],
    ...
    C: Copy + std::ops::Add<Output = C> + std::ops::Sub<Output = C> + std::ops::Mul<Output = C>,
{
    ...
    let mut k_start = 0;
    while k_start < sixteenth_groups {
        let k_end = (k_start + TILE_SIZE).min(sixteenth_groups);
        for j in 0..radix {
            let src_base = j * groups * 2;
...
/// Stockham passes.
#[inline]
pub(crate) fn stage_pair_impl<C, const TILE_SIZE: usize>(
    src: &[C],
    dst: &mut [C],
    radix: usize,
    first_twiddles: &[C],
