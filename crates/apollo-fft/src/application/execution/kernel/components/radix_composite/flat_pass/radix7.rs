        ...
        if prev_len == 1 {
            ...
            if g < g_count {
                ...
                unsafe {
                    ...
                    let a5 = cmul(
                        load_prefix::<T, A>(src, 5 * stride + at, prev_len, m),
                        load_prefix::<T, A>(tw, 4 * prev_len, prev_len, m),
                    );
                    let a6 = cmul(
                        load_prefix::<T, A>(src, 6 * stride + at, prev_len, m),
                        load_prefix::<T, A>(tw, 5 * prev_len, prev_len, m),
                    );
            ...
            let (t1, t2, t3, t4, t5, t6) = if run_over {
                ...
                unsafe {
                    (
                        ...
                        load::<T, A>(tw, 2 * prev_len + tail_j),
                        load::<T, A>(tw, 3 * prev_len + tail_j),
                        load::<T, A>(tw, 4 * prev_len + tail_j),
                        load_prefix::<T, A>(tw, 5 * prev_len + tail_j, rem, m),
                    )
                }
            } else {
                (
                ...
                if rem != 0 {
                    ...
                    if g + 1 < g_count && run_over {
                        ...
                        unsafe {
                            ...
                            let a5 = cmul(load::<T, A>(src, 5 * stride + at), t5);
                            let a6 = cmul(load::<T, A>(src, 6 * stride + at), t6);
                            let b = dft7(k, a0, a1, a2, a3, a4, a5, a6);
                            for arm in 0..7 {
