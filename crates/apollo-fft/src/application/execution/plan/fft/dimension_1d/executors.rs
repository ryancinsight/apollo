...
/// The 256-point two-pass base (ADR 0061): one kernel at n = 256, and two
/// or four blocks under one radix step at 512 and 1024, where the 128
/// route made four and eight.
...
pub(super) fn exec_base128_forward<F: MixedRadixScalar<Complex = Complex<F>>>(
    ...
    assert!(
        transform_via_base_128::<F, false, false>(slice, plan.base128_state()),
...
pub(super) fn exec_base128_inverse<F: MixedRadixScalar<Complex = Complex<F>>>(
    ...
    assert!(
        transform_via_base_128::<F, true, false>(slice, plan.base128_state()),
        "invariant: the selected base-128 capability remains available"
    );
