...
/// Lengths that no shaped strategy accepts still compute the DFT.
///
...
/// declines it. That used to reach Rader, whose primality precondition is
/// guarded only by a `debug_assert`, so release builds computed silently wrong
/// results: a forward-then-normalized-inverse round trip at 361 returned a
/// maximum error of 2.019e1 against an input of order 1, and
/// `apollo-dctdst`'s DCT-II at 361 (which routes through a 722-point FFT)
/// returned relative error 1.0101 — output uncorrelated with the truth,
/// reported as success. Bluestein now serves the tail of the chain, so the
/// length that exposed the defect is a correct route rather than a refused
/// one.
#[test]
fn composite_length_without_a_shaped_route_round_trips() {
    let n = 361;
    let plan = FftPlan1D::<f64>::new(Shape1D::new(n).expect("shape"));
    ...
    let max_err = original
        ...
        .map(|(a, b)| (*a - *b).norm())
        .fold(0.0f64, f64::max);
    // Three transforms at the padded length bound the error near `p * eps`;
    // the defect this replaces sat at 2.019e1, twelve orders above.
    assert!(max_err < 1e-9, "round trip at {n} drifted by {max_err:e}");
}
