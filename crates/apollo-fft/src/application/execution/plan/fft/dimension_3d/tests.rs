...
/// [`FftPlan3D::inverse_complex_unnorm_inplace`] (ADR 0067 slice 1) is the
/// normalized inverse without the final `1/N` scale: each axis pass of the
/// normalized inverse divides by its own axis length and rounds once doing
/// so, so `unnorm == normalized * N` up to a few `ε` relative to `N max|x|`.
/// `N` a power of two makes every axis length here a power of two, so each
/// division and its inverting multiplication is an exact binary rescale
/// (IEEE 754 exact for multiplication/division by a power of two absent
/// overflow/underflow) and the two sides must agree bit for bit.
#[test]
fn unnorm_inverse_equals_normalized_inverse_times_element_count() {
    use crate::application::execution::plan::fft::dimension_3d::FftPlan3D;
    use crate::domain::metadata::shape::Shape3D;

    for (nx, ny, nz) in [(4usize, 6usize, 8usize), (8, 8, 8)] {
        let plan = FftPlan3D::<f64>::new(
            Shape3D::new(nx, ny, nz).expect("invariant: shape lengths are non-zero"),
        );
        let original = Array3::from_shape_fn([nx, ny, nz], |[i, j, k]| {
        ...
        plan.forward_complex_inplace(&mut normalized);
        plan.inverse_complex_inplace(&mut normalized);

        let mut unnorm = original.clone();
        plan.forward_complex_inplace(&mut unnorm);
        plan.inverse_complex_unnorm_inplace(&mut unnorm);

        let peak = normalized.iter().fold(0.0f64, |m, v| m.max(v.norm()));
        let error = unnorm
            .iter()
            .zip(normalized.iter())
            .map(|(a, b)| (*a - *b * n).norm())
        ...
        if (nx * ny * nz).is_power_of_two() {
            assert_eq!(
                error, 0.0,
                "{nx}x{ny}x{nz}: power-of-two element count must scale exactly"
            );
        } else {
            let bound = 4.0 * f64::EPSILON * n * peak;
            assert!(
                error <= bound,
                "{nx}x{ny}x{nz}: unnorm vs normalized*N beyond {bound:e}, err={error:e}"
            );
        }
    }
}
