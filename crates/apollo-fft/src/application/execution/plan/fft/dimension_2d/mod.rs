...
pub(super) const RAYON_THRESHOLD: usize = 32768; // 32K Complex64 ≈ 512 KB

/// Tile size for cache-blocked transpose.
/// A 32×32 tile of Complex64 = 8 KB, fitting comfortably in L1 (32–48 KB).
pub(super) const TRANSPOSE_TILE: usize = 32;
...
pub(super) trait Plan2dReal32: Copy + UninitWorkspaceElement {
...
pub struct FftPlan2D {
    ...
    pub(super) twiddle_row_fwd_64: Option<Arc<[Complex64]>>,
    /// Per-stage inverse twiddle table for row (axis-1) passes of length `ny`.
    pub(super) twiddle_row_inv_64: Option<Arc<[Complex64]>>,
    /// Per-stage forward twiddle table for column (axis-0) passes of length `nx`.
    pub(super) twiddle_col_fwd_64: Option<Arc<[Complex64]>>,
    /// Per-stage inverse twiddle table for column (axis-0) passes of length `nx`.
    pub(super) twiddle_col_inv_64: Option<Arc<[Complex64]>>,
    /// f32 variants of the above four.
    pub(super) twiddle_row_fwd_32: Option<Arc<[Complex32]>>,
    pub(super) twiddle_row_inv_32: Option<Arc<[Complex32]>>,
    pub(super) twiddle_col_fwd_32: Option<Arc<[Complex32]>>,
    pub(super) twiddle_col_inv_32: Option<Arc<[Complex32]>>,
    ...
    pub(super) scratch_col_64: std::sync::Mutex<Vec<Complex64>>,
    /// Preallocated scratch buffer for the f32 column pass.
    pub(super) scratch_col_32: std::sync::Mutex<Vec<Complex32>>,
}
...
impl FftPlan2D {
    ...
    pub(super) fn pack_real32_complex<T: Plan2dReal32>(
        &self,
        input: &Array2<T>,
    ) -> Array2<Complex32> {
    ...
    pub(super) fn project_real32<T: Plan2dReal32>(&self, input: Array2<Complex32>) -> Array2<T> {
        let mut values = uninit_copy_vec(input.len());
        for (slot, value) in values.iter_mut().zip(input.iter()) {
