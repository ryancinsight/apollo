...
/// [`FftPlan2D::inverse_complex_unnorm_inplace`] (ADR 0067 slice 1) is the
...
/// `N` a power of two makes both axis lengths powers of two here, so each
...
fn unnorm_inverse_equals_normalized_inverse_times_element_count() {
    for (nx, ny) in [(6usize, 10usize), (8, 16)] {
        let plan = FftPlan2D::<f64>::new(Shape2D::new(nx, ny).expect("non-zero"));
        let original = Array2::from_shape_fn([nx, ny], |[i, j]| {
            let x = (i * ny + j) as f64;
            Complex64::new((0.17 * x).sin() + 0.3, 0.23 * (0.31 * x).cos())
        ...
        if (nx * ny).is_power_of_two() {
            assert_eq!(
                error, 0.0,
                "{nx}x{ny}: power-of-two element count must scale exactly"
            );
        } else {
            let bound = 4.0 * f64::EPSILON * n * peak;
            assert!(
                error <= bound,
                "{nx}x{ny}: unnorm vs normalized*N beyond {bound:e}, err={error:e}"
            );
        }
    }
}
