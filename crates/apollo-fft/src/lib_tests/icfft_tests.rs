...
fn owned_static_complex_wrappers_reuse_input_allocation() {
    ...
    let actual1 = fft_1d_complex_static::<f64, 16>(signal1);
    ...
    let actual_recovered1 = ifft_1d_complex_static::<f64, 16>(expected1);
    assert_eq!(
        ptr_recovered1,
    ...
    let actual2 = fft_2d_complex_static::<f64, 4, 5>(signal2);
    ...
    let actual_recovered2 = ifft_2d_complex_static::<f64, 4, 5>(expected2);
    assert_eq!(
        ptr_recovered2,
    ...
    let actual3 = fft_3d_complex_static::<f64, 3, 4, 5>(signal3);
    ...
    let actual_recovered3 = ifft_3d_complex_static::<f64, 3, 4, 5>(expected3);
    assert_eq!(
        ptr_recovered3,
    ...
    let actual32 = fft_1d_complex_static::<f32, 16>(signal32);
    ...
    let actual_recovered32 = ifft_1d_complex_static::<f32, 16>(expected32);
    assert_eq!(
        ptr_recovered32,
    ...
    let actual32_2d = fft_2d_complex_static::<f32, 4, 5>(signal32_2d);
    ...
    let actual_recovered32_2d = ifft_2d_complex_static::<f32, 4, 5>(expected32_2d);
    assert_eq!(
        ptr_recovered32_2d,
    ...
    let actual32_3d = fft_3d_complex_static::<f32, 3, 4, 5>(signal32_3d);
    ...
    let actual_recovered32_3d = ifft_3d_complex_static::<f32, 3, 4, 5>(expected32_3d);
    assert_eq!(
        ptr_recovered32_3d,
...
fn typed_static_complex_wrappers_match_dynamic_f32_paths() {
    ...
    fft_1d_complex_static_into::<f32, 16>(&signal1, &mut actual1);
    for (expected, actual) in expected1.iter().zip(actual1.iter()) {
        assert!((expected - actual).norm() < 2e-5);
    }
    ...
    ifft_1d_complex_static_into::<f32, 16>(&expected1, &mut actual_recovered1);
    for (expected, actual) in recovered1.iter().zip(actual_recovered1.iter()) {
        assert!((expected - actual).norm() < 2e-5);
    }
    fft_1d_complex_static_inplace::<f32, 16>(&mut actual_recovered1);
    ifft_1d_complex_static_inplace::<f32, 16>(&mut actual_recovered1);
    for (expected, actual) in recovered1.iter().zip(actual_recovered1.iter()) {
        assert!((expected - actual).norm() < 2e-5);
    }
    ...
    fft_2d_complex_static_into::<f32, 4, 5>(&signal2, &mut actual2);
    for (expected, actual) in expected2.iter().zip(actual2.iter()) {
        assert!((expected - actual).norm() < 3e-5);
    }
    ...
    ifft_2d_complex_static_into::<f32, 4, 5>(&expected2, &mut actual_recovered2);
    for (expected, actual) in recovered2.iter().zip(actual_recovered2.iter()) {
        assert!((expected - actual).norm() < 3e-5);
    }
    fft_2d_complex_static_inplace::<f32, 4, 5>(&mut actual_recovered2);
    ifft_2d_complex_static_inplace::<f32, 4, 5>(&mut actual_recovered2);
    for (expected, actual) in recovered2.iter().zip(actual_recovered2.iter()) {
        assert!((expected - actual).norm() < 3e-5);
    }
    ...
    fft_3d_complex_static_into::<f32, 3, 4, 5>(&signal3, &mut actual3);
    for (expected, actual) in expected3.iter().zip(actual3.iter()) {
        assert!((expected - actual).norm() < 4e-5);
    }
    ...
    ifft_3d_complex_static_into::<f32, 3, 4, 5>(&expected3, &mut actual_recovered3);
    for (expected, actual) in recovered3.iter().zip(actual_recovered3.iter()) {
        assert!((expected - actual).norm() < 4e-5);
    }
    fft_3d_complex_static_inplace::<f32, 3, 4, 5>(&mut actual_recovered3);
    ifft_3d_complex_static_inplace::<f32, 3, 4, 5>(&mut actual_recovered3);
    for (expected, actual) in recovered3.iter().zip(actual_recovered3.iter()) {
        assert!((expected - actual).norm() < 4e-5);
    }
}
...
fn typed_dynamic_complex_wrappers_match_f32_plans_and_reuse_owned_allocation() {
    ...
    let actual1_owned = fft_1d_complex_owned(owned1);
    ...
    let inverse1_owned = ifft_1d_complex_owned(owned_inverse1);
    ...
    let actual2_owned = fft_2d_complex_owned(owned2);
    ...
    let inverse2_owned = ifft_2d_complex_owned(owned_inverse2);
    ...
    let actual3_owned = fft_3d_complex_owned(owned3);
    ...
    let inverse2_owned = ifft_3d_complex_owned(owned_inverse3);
