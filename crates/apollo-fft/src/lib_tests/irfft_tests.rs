...
fn real_inverse_mutating_spectrum_wrappers_reuse_spectrum_storage() {
    ...
    let expected_recovered1 = ifft_1d_array::<f64>(&expected_spectrum1);
    ...
    let expected_recovered2 = ifft_2d_array::<f64>(&expected_spectrum2);
    let mut spectrum2 = expected_spectrum2.clone();
    let spectrum_ptr2 = spectrum2.as_slice().unwrap().as_ptr();
    let mut actual_recovered2 = Array2::<f64>::zeros([6, 10]);
    ifft_2d_array_into_spectrum_scratch(&mut spectrum2, &mut actual_recovered2);
    assert_eq!(spectrum_ptr2, spectrum2.as_slice().unwrap().as_ptr());
...
fn typed_real_inverse_mutating_spectrum_wrappers_reuse_spectrum_storage() {
    ...
    let expected_spectrum32 = fft_1d_array(&signal32);
    let expected_recovered32 = ifft_1d_array::<f32>(&expected_spectrum32);
    ...
    ifft_1d_array_into_spectrum_scratch::<f32>(&mut spectrum32, &mut actual_recovered32);
    ...
    let expected_spectrum3 = fft_3d_array(&field3);
    let expected_recovered3 = ifft_3d_array::<f32>(&expected_spectrum3);
    let mut spectrum3 = expected_spectrum3.clone();
    let spectrum_ptr3 = spectrum3.as_slice().unwrap().as_ptr();
    let mut actual_recovered3 = Array3::<f32>::zeros([3, 4, 5]);
    ifft_3d_array_into_spectrum_scratch::<f32>(&mut spectrum3, &mut actual_recovered3);
    assert_eq!(spectrum_ptr3, spectrum3.as_slice().unwrap().as_ptr());
