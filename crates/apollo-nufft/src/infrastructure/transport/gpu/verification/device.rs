...
mod tests {
    ...
    use crate::{
        nufft_type1_1d_fast, nufft_type1_3d, nufft_type1_3d_fast, nufft_type2_1d_fast,
        nufft_type2_3d, nufft_type2_3d_fast, UniformDomain1D, UniformGrid3D,
    ...
    fn nufft_wgpu_execution_suite_when_device_exists() {
        ...
        // 9. type1_3d_matches_cpu_exact_reference
        {
            let grid = UniformGrid3D::new(3, 2, 2, 0.5, 0.75, 1.0).expect("grid");
            let plan = NufftWgpuPlan3D::new(grid, 2, 6);
        ...
        // 10. leto_type1_3d_matches_slice_path
        {
            let grid = UniformGrid3D::new(3, 2, 2, 0.5, 0.75, 1.0).expect("grid");
            let plan = NufftWgpuPlan3D::new(grid, 2, 6);
            let positions = [(0.0_f32, 0.0, 0.0), (0.35, 0.7, 0.5), (1.1, 0.2, 1.4)];
        ...
        // 11. type1_3d_typed_mixed_storage_matches_represented_input
        {
            let grid = UniformGrid3D::new(3, 2, 2, 0.5, 0.75, 1.0).expect("grid");
            let plan = NufftWgpuPlan3D::new(grid, 2, 6);
            let positions = [(0.0_f32, 0.0, 0.0), (0.35, 0.7, 0.5), (1.1, 0.2, 1.4)];
