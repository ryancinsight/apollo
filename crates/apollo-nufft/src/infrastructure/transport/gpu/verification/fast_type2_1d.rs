...
use eunomia::{abs_diff_eq, Complex32, Complex64};
use leto::Array1;
...
fn fast_type2_1d_normalization_invariance() {
    ...
    for (index, (actual, expected)) in actual.iter().zip(expected.iter()).enumerate() {
        assert!(
            abs_diff_eq!(actual.re, expected.re, epsilon = 1e-4),
            "real mismatch at position index {index}: actual={actual:?}, expected={expected:?}"
        );
        assert!(
            abs_diff_eq!(actual.im, expected.im, epsilon = 1e-4),
    ...
    for (index, value) in actual.iter().enumerate() {
        assert!(
            abs_diff_eq!(value.re, reference.re, epsilon = 1e-4),
            "constancy regression at position index {index}: {value:?} vs reference {reference:?}"
        );
        assert!(
            abs_diff_eq!(value.im, reference.im, epsilon = 1e-4),
            "constancy regression at position index {index}: {value:?} vs reference {reference:?}"
        );
    }
}
