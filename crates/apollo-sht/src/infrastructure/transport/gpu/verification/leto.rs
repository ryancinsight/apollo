...
fn leto_strided_forward_matches_logical_slice_when_device_exists() {
    ...
    let samples = Array2::from_shape_fn(
        [plan.payload().latitudes(), plan.payload().longitudes()],
        |[lat, lon]| Complex32::new(lat as f32 + lon as f32 * 0.25, 0.5 + lon as f32 * 0.1),
    );
    let mut interleaved = Vec::with_capacity(plan.payload().sample_count() * 2);
    ...
    let interleaved_leto = leto::Array::from_mnemosyne_slice(
        [plan.payload().latitudes(), plan.payload().longitudes() * 2],
        &interleaved,
    )
