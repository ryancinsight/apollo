...
impl RadonGpuKernel {
    ...
    pub fn execute_backproject(
        ...
        let sinogram_buf = hep_device.upload(&sinogram_flat).map_err(|e| WgpuError::BufferMapFailed {
            message: e.to_string(),
        })?;
        let angle_buf = hep_device.upload(angles).map_err(|e| WgpuError::BufferMapFailed {
            message: e.to_string(),
        })?;
        let rows = plan.rows();
        let cols = plan.cols();
        let image_buf = hep_device.alloc_zeroed::<f32>(rows * cols).map_err(|e| WgpuError::BufferMapFailed {
            message: e.to_string(),
        })?;
    ...
    pub fn execute(
        ...
        let image_buffer = hep_device.upload(&image_data).map_err(|e| WgpuError::BufferMapFailed {
            message: e.to_string(),
        })?;
        let angle_buffer = hep_device.upload(angles).map_err(|e| WgpuError::BufferMapFailed {
            message: e.to_string(),
        })?;
        let sinogram_buffer = hep_device.alloc_zeroed::<f32>(plan.angle_count() * plan.detector_count()).map_err(|e| WgpuError::BufferMapFailed {
            message: e.to_string(),
        })?;
    ...
    pub fn execute_filtered_backproject(
        ...
        let filter_buf = hep_device.upload(&h).map_err(|e| WgpuError::BufferMapFailed {
            message: e.to_string(),
        })?;

        // Filtered sinogram buffer (pass 1 output, pass 2 input).
        let filtered_buf = hep_device.alloc_zeroed::<f32>(angle_count * detector_count).map_err(|e| WgpuError::BufferMapFailed {
            message: e.to_string(),
        })?;
        ...
        let fbp_filter_bg = device.inner().create_bind_group(&wgpu::BindGroupDescriptor {
        ...
        let backproject_bg = device.inner().create_bind_group(&wgpu::BindGroupDescriptor {
        ...
        hep_device.download(&image_buf, &mut raw).map_err(|e| WgpuError::BufferMapFailed {
            message: e.to_string(),
        })?;
